...
class PointServiceTest {
	...
	void charge() {
		...
		when(paymentService.pay(member, chargeAmount, token)).thenReturn(payResponse);

		//when
		pointService.charge(member.getEmail(), token, point.getValue());

		//then
		assertThat(member.getPoint()).isEqualTo(chargeAmount);
		verify(memberRepository).findByEmail(member.getEmail());
		verify(pointRepository).save(any(Point.class));
		verify(paymentService).pay(member, chargeAmount, token);
	}
}